...
When a webroot uses the PHP runtime, direnv also prepends a `.bin` directory to `PATH` containing a `php` symlink pointing to the configured version (e.g. `/usr/bin/php8.3`). This means running `php` inside a webroot directory always uses the version configured for that webroot, even when the system default is different.

```
$ cd /webroots/mysite
direnv: loading /webroots/mysite/.envrc
$ php -v
PHP 8.3.x (cli) ...

$ cd /webroots/other-site
direnv: loading /webroots/other-site/.envrc
$ php -v
PHP 8.5.x (cli) ...
```

The `.envrc` is auto-generated by the platform and looks like:
