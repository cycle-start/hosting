...
func (s *ValkeyUserService) Create(ctx context.Context, user *model.ValkeyUser, plaintextPassword string) error {
	user.PasswordHash = crypto.ValkeyPasswordHash(plaintextPassword)

	_, err := s.db.Exec(ctx,
		`INSERT INTO valkey_users (id, valkey_instance_id, username, password_hash, privileges, key_pattern, status, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
		user.ID, user.ValkeyInstanceID, user.Username, user.PasswordHash,
...
func (s *ValkeyUserService) ListByInstance(ctx context.Context, instanceID string, limit int, cursor string) ([]model.ValkeyUser, bool, error) {
	query := `SELECT id, valkey_instance_id, username, password_hash, privileges, key_pattern, status, status_message, created_at, updated_at FROM valkey_users WHERE valkey_instance_id = $1`
	args := []any{instanceID}
	argIdx := 2
	...
	for rows.Next() {
		var u model.ValkeyUser
		if err := rows.Scan(&u.ID, &u.ValkeyInstanceID, &u.Username, &u.PasswordHash,
...
func (s *ValkeyUserService) Update(ctx context.Context, user *model.ValkeyUser, plaintextPassword string) error {
	if plaintextPassword != "" {
		user.PasswordHash = crypto.ValkeyPasswordHash(plaintextPassword)
	}

	_, err := s.db.Exec(ctx,
		`UPDATE valkey_users SET username = $1, password_hash = $2, privileges = $3, key_pattern = $4, updated_at = now()
		 WHERE id = $5`,
		user.Username, user.PasswordHash, user.Privileges, user.KeyPattern, user.ID,
	)
	if err != nil {
