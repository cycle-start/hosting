...
func Generate(cfg *Config, outputDir string, progress ProgressFunc) (*GenerateResult, error) {
	...
	// 11. Generate Helm values override (replaces fragile --set flags)
	progress("Generating Helm values...")
	helmValues := generateHelmValues(cfg, controlplaneIP, outputDir)
	result.Files = append(result.Files, GeneratedFile{
		Path:    "generated/helm-values.yaml",
		Content: helmValues,
	})
...
func generateDotEnv(cfg *Config, envPath string) (string, error) {
	...
	// Control panel DB and JWT secret
	if cfg.ControlPlane.Database.Mode == "external" {
		db := cfg.ControlPlane.Database
		vars["CONTROLPANEL_DATABASE_URL"] = fmt.Sprintf("postgres://%s:%s@%s:%d/%s_controlpanel?sslmode=%s",
			db.User, db.Password, db.Host, db.Port, db.Name, db.SSLMode)
	} else {
		vars["CONTROLPANEL_DATABASE_URL"] = fmt.Sprintf("postgres://controlpanel:controlpanel@%s:5432/controlpanel", controlplaneIP)
	}
	if existing["CONTROLPANEL_JWT_SECRET"] != "" {
		vars["CONTROLPANEL_JWT_SECRET"] = existing["CONTROLPANEL_JWT_SECRET"]
	} else {
		jwtKey := make([]byte, 32)
		rand.Read(jwtKey)
		vars["CONTROLPANEL_JWT_SECRET"] = fmt.Sprintf("%x", jwtKey)
	}
	...
	if cfg.SSO.Mode == "internal" || cfg.SSO.Mode == "external" {
		...
		if existing["OAUTH2_PROXY_COOKIE_SECRET"] != "" {
			...
			vars["OAUTH2_PROXY_COOKIE_SECRET"] = fmt.Sprintf("%x", cookieSecret)
		}
	...
	section("Control panel DB (Postgres)",
		"CONTROLPANEL_DATABASE_URL")
	section("Control panel JWT signing secret (hex-encoded, 32 bytes)",
		"CONTROLPANEL_JWT_SECRET")
...
func generateAllGroupVars(cfg *Config, controlplaneIP, storageNodeIP, cephFSID, cephWebKey string) string {
	...
	b.WriteString(fmt.Sprintf("core_api_url: \"https://api.%s/api/v1\"\n", cfg.Brand.PlatformDomain))
...
func generateClusterYAML(cfg *Config, controlplaneIP string) string {
	...
	apiURL := fmt.Sprintf("https://api.%s/api/v1", cfg.Brand.PlatformDomain)
...
// generateHelmValues produces a Helm values override file that replaces the
// fragile --set flags in deploy-controlplane.sh. Values that contain special
// characters (like database URLs with @, =, etc.) are properly YAML-quoted.
func generateHelmValues(cfg *Config, controlplaneIP, outputDir string) string {
	var b strings.Builder

	// Read .env to get generated secrets (they were written in step 9)
	envPath := filepath.Join(outputDir, ".env")
	env := parseEnvFile(envPath)

	b.WriteString("# Generated by hosting setup wizard — do not edit manually.\n")
	b.WriteString("# This file is used by deploy-controlplane.sh as a Helm values override.\n\n")

	// Image config: local images built by deploy-controlplane.sh and imported into k3s
	b.WriteString("image:\n")
	b.WriteString("  pullPolicy: Never\n")
	b.WriteString("  coreApi:\n")
	b.WriteString("    repository: hosting-core-api\n")
	b.WriteString("  worker:\n")
	b.WriteString("    repository: hosting-worker\n")
	b.WriteString("  adminUi:\n")
	b.WriteString("    repository: hosting-admin-ui\n")
	b.WriteString("  mcpServer:\n")
	b.WriteString("    repository: hosting-mcp-server\n")
	b.WriteString("  controlpanelApi:\n")
	b.WriteString("    repository: controlpanel-api\n")
	b.WriteString("  controlpanelUi:\n")
	b.WriteString("    repository: controlpanel-ui\n\n")

	// All components use hostNetwork so they bind directly on the host
	b.WriteString("coreApi:\n")
	...
	b.WriteString("adminUi:\n")
	b.WriteString("  hostNetwork: true\n")
	// Admin UI needs SSO config and API key via extraEnv
	if cfg.SSO.Mode == "internal" || cfg.SSO.Mode == "external" {
		domain := cfg.Brand.PlatformDomain
		b.WriteString("  extraEnv:\n")
		b.WriteString(fmt.Sprintf("    - name: OIDC_AUTH_URL\n      value: %q\n", env["OIDC_AUTH_URL"]))
		b.WriteString(fmt.Sprintf("    - name: OIDC_TOKEN_URL\n      value: %q\n", env["OIDC_TOKEN_URL"]))
		b.WriteString(fmt.Sprintf("    - name: OIDC_ISSUER_URL\n      value: %q\n", env["OIDC_ISSUER_URL"]))
		b.WriteString(fmt.Sprintf("    - name: ADMIN_CLIENT_ID\n      value: %q\n", env["ADMIN_CLIENT_ID"]))
		b.WriteString("    - name: OIDC_CLIENT_SECRET\n")
		b.WriteString("      valueFrom:\n")
		b.WriteString("        secretKeyRef:\n")
		b.WriteString("          name: oidc-credentials\n")
		b.WriteString("          key: OIDC_CLIENT_SECRET\n")
		b.WriteString("          optional: true\n")
		b.WriteString(fmt.Sprintf("    - name: BASE_DOMAIN\n      value: %q\n", domain))
	}
	b.WriteString("\n")
	...
	// Config (non-secret, goes into ConfigMap)
	b.WriteString("config:\n")
	b.WriteString(fmt.Sprintf("  baseDomain: %q\n", cfg.Brand.PlatformDomain))
	b.WriteString(fmt.Sprintf("  regionId: %q\n", cfg.RegionName))
	b.WriteString(fmt.Sprintf("  clusterId: %q\n", cfg.ClusterName))
	b.WriteString(fmt.Sprintf("  lokiUrl: \"http://%s:3100\"\n", controlplaneIP))
	b.WriteString(fmt.Sprintf("  tenantLokiUrl: \"http://%s:3101\"\n", controlplaneIP))
	b.WriteString("\n")

	// Secrets (database URLs, tokens, keys)
	b.WriteString("secrets:\n")
	yamlSecret := func(key, value string) {
		if value != "" {
			b.WriteString(fmt.Sprintf("  %s: %q\n", key, value))
		}
	}
	yamlSecret("coreDatabaseUrl", env["CORE_DATABASE_URL"])
	yamlSecret("powerdnsDatabaseUrl", env["POWERDNS_DATABASE_URL"])
	yamlSecret("stalwartAdminToken", env["STALWART_ADMIN_TOKEN"])
	yamlSecret("secretEncryptionKey", env["SECRET_ENCRYPTION_KEY"])
	yamlSecret("controlpanelDatabaseUrl", env["CONTROLPANEL_DATABASE_URL"])
	yamlSecret("controlpanelJwtSecret", env["CONTROLPANEL_JWT_SECRET"])
	yamlSecret("hostingApiKey", cfg.APIKey)
	// SSH CA private key — read from file if it exists
	sshCAPath := filepath.Join(outputDir, "ssh_ca")
	if sshCAData, err := os.ReadFile(sshCAPath); err == nil && len(sshCAData) > 0 {
		b.WriteString("  sshCaPrivateKey: |\n")
		for _, line := range strings.Split(strings.TrimRight(string(sshCAData), "\n"), "\n") {
			b.WriteString("    " + line + "\n")
		}
	}
	b.WriteString("\n")
	...
	b.WriteString(fmt.Sprintf("  address: \"%s:7233\"\n\n", controlplaneIP))

	// Ingress extra hosts (Temporal UI, Grafana, Prometheus, Loki)
	domain := cfg.Brand.PlatformDomain
	b.WriteString("ingress:\n")
	b.WriteString("  extraHosts:\n")
	b.WriteString(fmt.Sprintf("    - host: temporal.%s\n", domain))
	b.WriteString("      service: temporal-ui\n")
	b.WriteString("      port: 8233\n")
	b.WriteString(fmt.Sprintf("    - host: grafana.%s\n", domain))
	b.WriteString("      service: grafana\n")
	b.WriteString("      port: 3000\n")
	b.WriteString(fmt.Sprintf("    - host: prometheus.%s\n", domain))
	b.WriteString("      service: prometheus\n")
	b.WriteString("      port: 9090\n")
	b.WriteString(fmt.Sprintf("    - host: loki.%s\n", domain))
	b.WriteString("      service: loki\n")
	b.WriteString("      port: 3100\n")
