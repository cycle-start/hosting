...
func Generate(cfg *Config, outputDir string, progress ProgressFunc) (*GenerateResult, error) {
	...
	// 8. Generate .env file for deploy-controlplane.sh
	progress("Generating .env file...")
	dotEnv := generateDotEnv(cfg)
	result.Files = append(result.Files, GeneratedFile{
		Path:    "generated/.env",
		Content: dotEnv,
	})
...
func generateDotEnv(cfg *Config) string {
	var b strings.Builder

	b.WriteString("# Generated by hosting setup wizard\n\n")
	b.WriteString(fmt.Sprintf("BASE_DOMAIN=%s\n", cfg.Brand.PlatformDomain))
	b.WriteString(fmt.Sprintf("HOSTING_API_KEY=%s\n", cfg.APIKey))
	b.WriteString(fmt.Sprintf("STALWART_ADMIN_TOKEN=%s\n", cfg.Email.StalwartAdminToken))
	...
	if cfg.ControlPlane.Database.Mode == "external" {
		...
		b.WriteString(fmt.Sprintf("CORE_DATABASE_URL=%s\n", dsn))
	}

	// SSO (Azure AD)
	if cfg.SSO.Enabled && cfg.SSO.ClientID != "" {
		b.WriteString(fmt.Sprintf("\n# SSO (Azure AD / Entra ID)\n"))
		b.WriteString(fmt.Sprintf("OIDC_TENANT_ID=%s\n", cfg.SSO.TenantID))
		b.WriteString(fmt.Sprintf("OIDC_CLIENT_ID=%s\n", cfg.SSO.ClientID))
		b.WriteString(fmt.Sprintf("OIDC_CLIENT_SECRET=%s\n", cfg.SSO.ClientSecret))

		// Generate a random cookie secret for oauth2-proxy
		cookieSecret := make([]byte, 32)
		rand.Read(cookieSecret)
		b.WriteString(fmt.Sprintf("OAUTH2_PROXY_COOKIE_SECRET=%s\n",
			base64.StdEncoding.EncodeToString(cookieSecret)))
	}
