...
{{ end -}}
    ...
    access_log /home/{{ .TenantName }}/logs/{{ .WebrootName }}-access.log;
    error_log  /home/{{ .TenantName }}/logs/{{ .WebrootName }}-error.log;
...
func (m *NginxManager) GenerateConfig(webroot *agentv1.WebrootInfo, fqdns []*agentv1.FQDNInfo) (string, error) {
	tenantName := webroot.GetTenantName()
	webrootName := webroot.GetName()
	rt := webroot.GetRuntime()
	rtVersion := webroot.GetRuntimeVersion()
	publicFolder := webroot.GetPublicFolder()
	...
	for _, f := range fqdns {
		serverNames = append(serverNames, f.GetFqdn())
		if f.GetSslEnabled() {
			hasSSL = true
			if sslFQDN == "" {
				sslFQDN = f.GetFqdn()
	...
	// Build the document root path.
	docRoot := filepath.Join("/var/www/storage", tenantName, webrootName)
	if publicFolder != "" {
...
func (m *NginxManager) InstallCertificate(ctx context.Context, cert *agentv1.CertificateInfo) error {
	fqdn := cert.GetFqdn()
	...
	if err := os.WriteFile(privkeyPath, []byte(cert.GetKeyPem()), 0600); err != nil {
